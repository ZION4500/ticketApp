...
async function holdSeats(showtimeId, seatIds, sessionId) {
  await Seat.releaseExpiredHolds(showtimeId);
  const seats = await Seat.findByIds(seatIds);
...
async function releaseSeats(seatIds, sessionId) {
  const seats = await Seat.findByIds(seatIds);
  const released = [];
  for (const seat of seats) {
    if (seat.heldBy === sessionId && seat.status === 'held') {
      released.push(await Seat.update(seat.id, { status: 'available', heldBy: null, heldUntil: null }));
    }
  }
  return released;
}

async function confirmBooking({ showtimeId, seatIds, sessionId, customerName, customerEmail }) {
